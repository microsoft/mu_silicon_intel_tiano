  ...
  )
{
  ...
  if (Item != NULL) {
    ...
    Item->Header.LogType   = LShiftU64 (1, EventType);    // MU_CHANGE
  ...
  )
{
  ...
  for (Index = 0; Index < VTD_LOG_PEI_PRE_MEM_BAR_MAX; Index++) {
    ...
    if (CallbackHandle) {
      ...
      Event.Header.LogType = LShiftU64 (1, VTDLOG_PEI_PRE_MEM_DMA_PROTECT);   // MU_CHANGE
      Event.Data1          = InfoBuffer[Index].BarAddress;
      Event.Data2          = InfoBuffer[Index].Mode;
      Event.Data2         |= LShiftU64 (InfoBuffer[Index].Status, 8);   // MU_CHANGE
